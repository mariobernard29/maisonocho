...
import { Check, Printer, Calendar, Clock, MapPin, CreditCard, ChevronRight } from 'lucide-react';
    ...
    return (
      <div className="flex flex-col min-h-screen bg-crema items-center justify-center">
        <div className="animate-pulse text-center">
          <span className="editorial-title text-4xl text-olive">MAISON VIII</span>
          <p className="text-xs text-gold tracking-widest mt-2">Cargando su ticket...</p>
        </div>
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="flex flex-col min-h-screen bg-crema items-center justify-center text-center p-4">
        <h2 className="editorial-title text-3xl text-olive font-light">No encontramos su pedido</h2>
        <p className="text-sm text-olive/60 mt-2">Si acaba de realizar una orden, puede revisar su historial.</p>
        <Link
          href="/menu"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-crema">
      ...
      <main className="flex-grow mx-auto max-w-2xl w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Ticket Container */}
        <div id="print-ticket" className="glass-panel-gold rounded-lg shadow-xl p-6 sm:p-8 bg-white/80 border border-gold/25 relative space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="space-y-1">
              <span className="text-[10px] text-olive/40 uppercase tracking-wider block">Cliente</span>
              <span className="font-semibold text-olive block">{order.client_name}</span>
              <span className="text-olive/75 block">{order.client_phone}</span>
            </div>
            <div className="space-y-1 text-right">
              <span className="text-[10px] text-olive/40 uppercase tracking-wider block">Método de Pago</span>
              <span className="font-semibold text-olive uppercase block">
                ...
                {order.payment_method === 'link_pago' && 'Link de Pago (Tarjeta)'}
              </span>
          ...
          <div className="border-t border-b border-olive/5 py-4 space-y-2 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-olive font-medium">{order.delivery_date}</span>
              </div>
              <div className="flex items-center gap-1.5 justify-end">
                <Clock className="w-4 h-4 text-gold" />
                <span className="text-olive font-medium">{order.delivery_time_slot}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <span className="text-[10px] text-olive/40 uppercase tracking-wider block">Detalle del Pedido</span>
            <div className="divide-y divide-olive/5">
              {order.items?.map((item, idx) => (
                <div key={idx} className="py-2.5 flex justify-between gap-3 text-xs">
                  ...
                  <span className="font-semibold text-olive text-right">${(item.price * item.quantity).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-olive/10 pt-4 space-y-2.5 text-xs">
            ...
            <div className="flex justify-between items-center text-sm border-t border-olive/5 pt-2.5">
              <span className="editorial-title text-base text-olive font-semibold">Total Neto</span>
              <span className="font-bold text-olive text-lg">${order.total.toFixed(2)}</span>
            </div>
          </div>
          ...
          {order.notes && (
            <div className="bg-beige/40 p-3 rounded border border-olive/5 text-[11px] text-olive/75">
              <span className="font-bold block mb-0.5">Comentarios especiales:</span>
  ...
  return (
    <Suspense fallback={
      <div className="flex flex-col min-h-screen bg-crema items-center justify-center">
        <div className="animate-pulse text-center">
          <span className="editorial-title text-4xl text-olive">MAISON VIII</span>
          <p className="text-xs text-gold tracking-widest mt-2">Cargando...</p>
        </div>
      </div>
    }>
      <SuccessContent />
