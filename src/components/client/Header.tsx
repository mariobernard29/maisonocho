    ...
    <header className="sticky top-0 z-50 w-full glass-panel border-b border-olive/5 transition-all duration-300">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-10">
            <Link href="/" className="text-sm font-medium tracking-wide text-charcoal hover:text-gold transition-colors duration-300">
              Inicio
            </Link>
            <Link href="/menu" className="text-sm font-medium tracking-wide text-charcoal hover:text-gold transition-colors duration-300">
              Menú Boutique
            </Link>
          </nav>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-55 overflow-hidden">
          <div className="absolute inset-0 bg-charcoal/40 backdrop-blur-sm transition-opacity duration-500" onClick={() => setIsCartOpen(false)} />
            ...
            <div className="w-screen max-w-md bg-crema flex flex-col border-l border-gold/10 shadow-2xl animate-fade-in-up">
              <div className="flex-1 py-6 overflow-y-auto px-4 sm:px-6">
                <div className="flex items-start justify-between border-b border-olive/5 pb-5">
                  <h2 className="editorial-title text-2xl font-light text-olive">Su Carrito</h2>
                  <button
                ...
                <div className="mt-8">
                  {itemCount === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                      ...
                      <p className="editorial-title text-xl text-olive/60 font-light">El carrito está vacío</p>
                      <p className="text-xs text-olive/40 mt-1">Descubra nuestras deliciosas creaciones y añádalas aquí.</p>
                      <Link
                        href="/menu"
                        onClick={() => setIsCartOpen(false)}
                            ...
                            <div className="ml-4 flex-1 flex flex-col">
                              <div>
                                <div className="flex justify-between text-sm font-medium text-olive">
                                  <h3 className="editorial-title text-base font-medium">{item.product.name}</h3>
                                  <p className="ml-4 font-semibold text-olive">${(item.singleUnitPrice * item.quantity).toFixed(2)}</p>
                                </div>
                              ...
                              <div className="flex-1 flex items-end justify-between text-xs">
                                <div className="flex items-center border border-olive/10 rounded overflow-hidden">
                                  ...
                                  <span className="px-2.5 font-medium text-olive">{item.quantity}</span>
                                  <button
                                    onClick={() => cart.updateQuantity(item.cartId, item.quantity + 1)}
                                    className="p-1 text-olive/60 hover:bg-beige transition-colors"
                                  >
              ...
              {itemCount > 0 && (
                <div className="border-t border-olive/5 py-6 px-4 sm:px-6 bg-beige/30">
                  ...
                  <p className="mt-0.5 text-xs text-olive/50">Costo de envío calculado en el checkout.</p>
                  <div className="mt-6">
                    <Link
                      href="/checkout"
                      onClick={() => setIsCartOpen(false)}
